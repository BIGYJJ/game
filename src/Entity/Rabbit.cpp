...
void RabbitManager::renderTooltip(sf::RenderWindow& window, Rabbit* rabbit) {
    ...
    lines.push_back({rabbit->getName(), sf::Color(255, 200, 100)});
    lines.push_back({"Lv.1 普通", sf::Color(200, 200, 200)});
    ...
    if (rabbit->isAggressive()) {
        ...
        if (!stateStr.empty()) {
            lines.push_back({stateStr, sf::Color(255, 150, 100)});
        }
    }
    ...
    for (const auto& line : lines) {
        measureText.setString(sf::String::fromUtf8(line.first.begin(), line.first.end()));
        float width = measureText.getLocalBounds().width;
        if (width > maxWidth) maxWidth = width;
    }
    
    float tooltipWidth = std::max(140.0f, maxWidth + padding * 2);
    ...
    for (size_t i = 0; i < lines.size(); i++) {
        ...
        sf::Text text;
        text.setFont(font);
        text.setString(sf::String::fromUtf8(lines[i].first.begin(), lines[i].first.end()));
