...
void GameState::initRabbits() {
    ...
    for (auto& rabbit : rabbitManager->getRabbits()) {
        rabbit->setOnAttack([this](Rabbit& r) {
            ...
            player->getStats().modifyHealth(-actualDamage);
            
            if (eventLogPanel) {
                eventLogPanel->addMessage(r.getName() + " 攻击了你! -" + 
                                         std::to_string(static_cast<int>(actualDamage)) + " HP", 
                                         EventType::Combat);
            }
            
            std::cout << "[Combat] Rabbit attacked player for " << actualDamage << " damage" << std::endl;
